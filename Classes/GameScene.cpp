...
void Game::setSprite()
{
    ...
    else if (strcmp(spriteName.c_str(), defaultName5.c_str()) == 0)
    {
        // にんじん画像の配置
        this->addChild(UnagiSprite::create(), Z_Sprite, T_Unagi);
        root = 5;
    }
    else
    {
...
void Game::setBorder()
{
    ...
    Sprite *arrowItem = Sprite::create("yajirusi.png");
    
    lineLabel1->setPosition(border, WINSIZE.height/1.05);
    arrowItem->setPosition(border, WINSIZE.height/1.15);
    
    this->addChild(lineLabel1, Z_Label);
    this->addChild(arrowItem, Z_Label);
    
    lineLabel1->runAction(action4(border));
    arrowItem->runAction(action5(border));
    log("目印配置");
}
