...
{
    Sprite *bg = Sprite::create("resultBg.png");
...
void SetSprite::setButton()
{
    ...
    Menu *menu4 = Menu::create(ninjinItem, nullptr);
    ...
    // うなぎボタン
    MenuItem *unagiItem = MenuItemImage::create("startbtn_result.png", "startbtn_push.png", CC_CALLBACK_0(SetSprite::setUnagi, this));
    unagiItem->setPosition(Vec2(WINSIZE.width/2 + WINSIZE.width/3, WINSIZE.height/2 + WINSIZE.height/3 -200));
    
    Menu *menu5 = Menu::create(unagiItem, nullptr);
    menu5->setPosition(Vec2::ZERO);
    
    this->addChild(menu5, Z_Button);
    
    Label *unagiLabel = Label::createWithSystemFont("うなぎ", "ArialRoundedMTBold", 30);
    unagiLabel->setPosition(unagiItem->getPosition());
    unagiLabel->setScale(1.2);
    this->addChild(unagiLabel, Z_Label);
    ...
    Menu *menu6 = Menu::create(endItem, nullptr);
    ...
    Menu *menu7 = Menu::create(resetItem, NULL);
    menu7->setPosition(Vec2::ZERO);
    this->addChild(menu7, Z_Button);
...
void SetSprite::displayScore()
{
    log("スコア判定");
    UserDefault *userDef = UserDefault::getInstance();
    long score = userDef->getIntegerForKey("Score");
    long rank1 = userDef->getIntegerForKey("Score1");
    long rank2 = userDef->getIntegerForKey("Score2");
    long rank3 = userDef->getIntegerForKey("Score3");
